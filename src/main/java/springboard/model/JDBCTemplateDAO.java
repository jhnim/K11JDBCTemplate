...
public class JDBCTemplateDAO {
	...
	public int getTotalCount(Map<String, Object> map) {
		...
		//쿼리문에서 count(*)를 통해 반환하는 값을 정수형태로 반환한다.
	...
	public ArrayList<SpringBbsDTO> list(Map<String, Object> map){
		...
		sql += " ORDER BY bgroup DESC, bstep ASC"; //답변형일떄 정렬방법
	...
	public void write(final SpringBbsDTO springBbsDTO) {
		/*
		 매개변수로 전달되는 값을 익명클래스 내에서 사용할때는 반드시 final로 선언하여
		 값의 변경이 불가능하게 해줘야한다.
		...
		template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				/*
				 ...
				 또한 nextval은 한 문장에서 여러번 사용해도 시퀀스를 반환한다.
				 */
	...
	public SpringBbsDTO view(String idx) {
		...
		try {
			...
			// 0이나 33등 두자리 의 idx의 값으로 주면 500에러 발생하기 때문에 try~catch문으로 감싸야함.
	...
	/*
	 답변글을 입력하기 전에 현재 step보다 큰 게시물들을 일괄적으로 
