...
/*
 @Autowired
 	...
 	에러가 발생되어 서버를 시작할 수 없음.
 	
 	- 생성자, 멤버변수, 메소드(setter)에 적용가능함.
 	- 타입을 이용해 자동으로 프로퍼티 값을 설정함.
 	- 해당 어노테이션은 멤버변수에만 적용할 수 있다.
 		함수 내의 지역변수에서는 사용할 수 없다.
 	- 타입을 통해 자동으로 설정되므로 같은 타입이 2개 이상 존재하면 예외가 발생됨.
 */
@Controller
public class BbsController {
	...
	public String list(Model model, HttpServletRequest req) {
		/*
		 사용자로부터 받은 모든 요청은 request내장객체레 저장되고, 이를 커맨드 객체로
